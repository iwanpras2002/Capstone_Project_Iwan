...
def plot_report(data):
    ...
    ax1 = pd.concat([df_fraud,df_nfraud],axis=1).plot.bar(stacked = pd.concat([df_fraud,df_nfraud],axis=1).stack().all(),color =['#c34454','#53a4b1'],title = "Police Report Availability", figsize=(8, 6))
...
def plot_baru1(data):
    ...
    fraud_data = data[data['fraud_reported'] == "Y"]
    city_froud = pd.crosstab(index=fraud_data['incident_city'],columns='Count').sort_values(by='Count')

    ax = city_froud.plot.barh(title = "Fraud Reported by City group", 
    legend= False, 
    color = '#c34454', 
    figsize=(9, 7))
...
def plot_box(data):
    ...
    box.boxplot(column='total_claim_amount',by='incident_city', color = '#c34454', figsize=(9, 7))
    plt.xlabel("City")
